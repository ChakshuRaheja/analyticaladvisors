...
const CoveredCallReport = ({ 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch CC analysis data');
        ...
        console.error('Error fetching CC analysis data:', err);
  ...
  const filteredData = useMemo(() => {
    ...
    const filtered = reportData.filter((item) => {
      ...
      // In the filter function, replace the expiry match logic with:
const itemExpiry = item.Expiry ? item.Expiry.toString().trim().toLowerCase() : '';
const filterExpiry = expiryFilter ? expiryFilter.toString().trim().toLowerCase() : '';
const expiryMatch = !filterExpiry || itemExpiry === filterExpiry;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading CC analysis data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!reportData || reportData.length === 0) {
    return (
      <div className="bg-gray-50 p-6 rounded-lg text-center">
        <p className="text-gray-600">No CC analysis data available</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-1 text-left">Covered Call Analysis</h2>
